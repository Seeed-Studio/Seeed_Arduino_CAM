...
#include "sensors/include/ov3660.h"
#include "sensors/include/ov3660_regs.h"
#include "sensors/include/ov3660_settings.h"
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    if(framesize > FRAMESIZE_QXGA){
        printf("Invalid framesize: %u", framesize);
        return -1;
    }
    ...
    if (ret == 0) {
        printf("Set framesize to: %ux%u", w, h);
    }
    return ret;

fail:
    sensor->status.framesize = old_framesize;
    printf("Setting framesize to: %ux%u failed", w, h);
    return ret;
}
...
static int set_aec_value(sensor_t *sensor, int value)
{
    ...
    if (max_val < 0) {
        printf("Could not read max aec_value");
        return -1;
    }
    if (value > max_val) {
        value =max_val;
    ...
    if (ret == 0) {
        printf("Set aec_value to: %d / %d", value, max_val);
...
static int set_ae_level(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        printf("Set ae_level to: %d", level);
