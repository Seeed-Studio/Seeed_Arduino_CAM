...
static int calc_sysclk(int xclk, bool pll_bypass, int pll_multiplier, int pll_sys_div, int pre_div, bool root_2x, int pclk_root_div, bool pclk_manual, int pclk_div)
{
    ...
    CAM_INFO("Calculated XVCLK: %d Hz, REFIN: %u Hz, VCO: %u Hz, PLL_CLK: %u Hz, SYSCLK: %u Hz, PCLK: %u Hz", xclk, REFIN, VCO, PLL_CLK, SYSCLK, PCLK);
    return SYSCLK;
}
...
static int set_pll(sensor_t *sensor, bool bypass, uint8_t multiplier, uint8_t sys_div, uint8_t pre_div, bool root_2x, uint8_t pclk_root_div, bool pclk_manual, uint8_t pclk_div){
    ...
    if(multiplier > 252 || multiplier < 4 || sys_div > 15 || pre_div > 8 || pclk_div > 31 || pclk_root_div > 3){
        CAM_ERROR("Invalid arguments");
        return -1;
    }
    if(multiplier > 127){
        multiplier &= 0xFE;//only even integers above 127
    }
    CAM_INFO("Set PLL: bypass: %u, multiplier: %u, sys_div: %u, pre_div: %u, root_2x: %u, pclk_root_div: %u, pclk_manual: %u, pclk_div: %u", bypass, multiplier, sys_div, pre_div, root_2x, pclk_root_div, pclk_manual, pclk_div);
    ...
    if(ret){
        CAM_ERROR("set_sensor_pll FAILED!");
    }
    return ret;
}
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    if(framesize > FRAMESIZE_QSXGA){
        CAM_ERROR("Invalid framesize: %u", framesize);
        return -1;
    }
...
fail:
    ...
    CAM_ERROR("Setting framesize to: %ux%u failed", w, h);
    return ret;
}
...
static int set_agc_gain(sensor_t *sensor, int gain)
{
    ...
    if (ret == 0) {
        CAM_DEBUG("Set agc_gain to: %d", gain);
...
static int set_aec_value(sensor_t *sensor, int value)
{
    ...
    if (max_val < 0) {
        CAM_ERROR("Could not read max aec_value");
        return -1;
    }
    if (value > max_val) {
        value =max_val;
    ...
    if (ret == 0) {
        CAM_DEBUG("Set aec_value to: %d / %d", value, max_val);
...
static int set_ae_level(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        CAM_DEBUG("Set ae_level to: %d", level);
...
static int set_gainceiling(sensor_t *sensor, gainceiling_t level)
{
    ...
    if (ret == 0) {
        CAM_DEBUG("Set gainceiling to: %d", l);
...
static int set_denoise(sensor_t *sensor, int level)
{
    ...
    if (ret == 0) {
        CAM_DEBUG("Set denoise to: %d", level);
